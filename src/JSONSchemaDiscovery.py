# SchemaDiscovery.py -- JSON Analyzer With List Support
#
# for Python 2.7
#
# This module reads a JSON file or list of files of the same JSON type and derives a structure
# Use the output of this program to write a fail-proof data model
# This program uses recursion !!!
# If you set it all_same : then a whole batch of JSON files are analyzed in one structure use this if your inserts are failing
# If you set to all_diff : each file is analyzed separately
# If you set all_same and you send it a whole bunch of different JSON files, you get useless spaghetti!
#
#
# Usage: python SchemaDetector.py all_diff ../data/*
# Usage: python SchemaDetector.py all_same ../data/test_twitter.json
# Usage: python SchemaDetector.py all_same ../Volumes/data/twitter_json/twitter-2011-*s.json
#
# @author dominic Suciu <dominic@spacecurve.com>
# @copyright (C) SpaceCurve, Inc. 2012
# @todo
# - 
# - 
...
def parse_args():
    usage = "usage: %prog [options] inFileLst or FileLst Generator"  ## Edit Me !!
    ...
    parser.add_option("-v", "--verbose",
                      ...
                      default=False,
                      help="Heavy Logging")
    parser.add_option("-f", "--ifn",
                      dest="ifn",
                      metavar="<infilename>",
                      help="Geojson file suitable for input")
    parser.add_option("-c", "--schema_name",
                      dest="schema_name",
                      metavar="<schema_name>",
                      default = 'schema',
                      help="schema name")
    ...
    parser.add_option("-s", "--sampler",
                      ...
                      metavar="<sampler>",
                      default = 1,
                      help="sample objects take every s records")
    parser.add_option("-l", "--limit",
                      dest="limit",
                      metavar="<limit>",
                      default = None,
                      help="process first l records")
    parser.add_option("-a", "--attributes_to_lowercase",
                      dest="attributes_to_lowercase",
                      action="store_true",
                      metavar="<convert attributes_to_lowercase>",
                      default = False,
                      help="Convert all attributes to lower case. This implies data conversion!!")
...
class numberHisto:
    ...
    def export(self):
        if self.dataType == 'int':
            rstr = '[%s n=%i avg=%i min=%i max=%i]'%(self.dataType, self.n, self.sum/self.n, self.min, self.max)
        else:
            rstr = '[%s n=%i avg=%.2f min=%.2f max=%.2f]'%(self.dataType, self.n, self.sum/self.n, self.min, self.max)
        return rstr
...
class strHisto:
    ...
    def Add(self, instr):
        ...
        if self.min > val:
            self.min = val
    def export(self):
        rstr = '[%s n=%i avg_len=%.1f min=%i max=%i]'%(self.dataType, self.n, self.sum/self.n, self.min, self.max)
        return rstr
...
class boolHisto:
    ...
    def export(self):
        rstr = '[%s n=%i T/F:%i/%i]'%(self.dataType, self.n, self.n_true, self.n_false)
        return rstr
...
def ToPointLst(geoType, cLst):
    ...
    if geoType == 'LineString':
        ...
        return cLst
    elif geoType in ['Polygon_w_Holes', 'MultiPolygon', 'MultiPolygon_w_Holes']:
        for c in cLst:
...
def CharacterizeGeometry(gg):
    ...
    if gtype == 'Point':
        ...
        if len(g_coords) > 1:
            ...
            for i in range(len(g_coords)):
                ...
                if hole:
                    ...
                    pLst.append([c])
                print ('Polygon: part %i has %i points: Hole: %s'%(i, len(c), hole))
        ...
        for i in range(len(g_coords)):
            polygon = g_coords[i]
            ret = CharacterizeGeometry({'type':'Polygon', 'coordinates':polygon})
            if 'Holes' in ret[0]:
            ...
            pLst.extend(coords)
            lenc = ret[1]
            print ('MultiPolygon: part %i is %s has %i points'%(i, gtype, lenc))
            if lenc > geo_maxlen:
                geo_maxlen = lenc
        return gtype, geo_maxlen, pLst, holeLst
    else:
        sys.stderr.write("We do not support this geoJSON Type %s\n" %str(gg))
...
class geometryHisto:
    ...
    def addCoords(self, geoType, geoObjLst):
        ...
        for c in cLst:
            ...
            if self.Lo_max == None or self.Lo_min == None or self.La_max == None or self.La_min == None:
                self.Lo_max = Lo
                self.Lo_min = Lo
                self.La_max = La
                self.La_min = La
    ...
    def BoundBox(self):
        ...
        rStr = "BoundBox: (c= %i) [[%0.4f, %0.4f],[%0.4f, %0.4f]]"%(self.num_coords, self.Lo_min, self.La_min, self.Lo_max, self.La_max)
        return rStr
    ...
    def add(self, gObj):
        ...
        if max_numcoords == None:
            #This means that I was not able to run ImportedConvertToSupportedGeometry
            max_numcoords = 0
            if geoType == 'Polygon':
    ...
    def GenerateDDL(self, geo_type = 'geography', DumpLastComma=False):
        ...
        if DumpLastComma:
            ...
            comma = ','
        if self.Nullable:
            v = """"%s" %s%s  NULL, -- Choose geometry/geography"""%(self.attribute_name, geo_type, comma)
        else:
            v = """"%s" %s%s  -- Choose geometry/geography"""%(self.attribute_name, geo_type, comma)
    ...
    def export(self):
        ...
        rstr = '[%s n=%i %s points: (min=%i max=%i)] numPolygonsWithHoles: %i // %s'%(self.dataType, self.n, tstr, self.min, self.max, self.Holes, self.BoundBox())
        return rstr


class DataNode:
    """DataNode: Hierarchical data structure element"""
    def __init__(self, key, table_name='table_name', parent = None):
        if not(parent):
            parent_path = 'root'
        else:
    ...
    def PropagateNumRecs(self, n=None):
        ...
        self.parent_n = n
        if self.parent_n != self.n:
            print ('%s set by Propagation parent_n %i n %i'%(self, self.parent_n, self.n))
            self.Nullable = True
        if 'NoneType' in self.typeLst:
            print ('%s set by Propagation NoneType in typeLst: %s'%(self, str(self.typeLst)))
            self.Nullable = True
    ...
    def add(self, key, dictorobj):
        ...
        #1. add appropriate data histograms to keep track of and characterize built_in types like ['int', 'float', 'str', 'unicode', 'NoneType', 'bool', 'geometry']
        #2. Hold child nodes for embedded types
        #3. Deal with geometry which is a strange turkey that is a builtin that looks on the surface like a dictionary
        key = key#unicode(key)
        ...
        if key in self.d.keys():
            ...
            if key == 'geometry' or self.DetermineType(dictorobj) == 'geometry':
                ...
                try:
                    ...
                    print ("Tried to load this:\n %s \n\n Into this: %s" %(str(dictorobj), str(self.d[key])))
                    print ("It is very likely that this attribute has a mixture of geometry and multiple other types. Geometry does not play nice with other types: NOT Supported . . You Die!!!")
                    sys.exit(0)
            else:
                try:
            ...
            if key == 'geometry' or self.DetermineType(dictorobj) == 'geometry':
                ...
                self.d[key].histo = self.d[key] #this is why it is so weird for geometry types: It is a dict that MUST not spawn a node because it is a builtin This is where my typing model is broken
                self.d[key].add(dictorobj)
                return None
            else:
        ...
        #DataHistogram
        if dataType in ['int', 'float', 'str', 'unicode', 'NoneType', 'bool']:
    ...
    def DetermineTypeLst_ListSupport(self, inLst):
        ...
        for obj in inLst:
            ...
            if not(t in tLst):
                tLst.append(t)
        return tLst
    def JSONSchemaView(self):
        """JSONSchemaView: This fires off the JSON schema view"""
        #to be run from root
        ...
        for n in range(len(nLst)):
            ...
            node = nLst[n]
            if len(node.typeLst)==1 and node.Generate_DDL_TableTypeOrScalar() == 'type':
                outerNode = node
                node_index = n
                break
        if node_index == None:
        ...
        #This lets all the child nodes know how many records have been processed by the system
        #Discrepancies here mean that an attribute may need to be made nullable
        outerNode.PropagateNumRecs()
        #Feature type
        oStr = """CREATE TYPE %s.feature IS WHEN "Feature" THEN UNIT;\n"""%schemaName
        ...
        for n in range(node_index+1, len(nLst), 1):
            ...
            if node.Generate_DDL_TableTypeOrScalar() == 'type':
                typStr = node.GenerateDDL_CreateType(schemaName, tableName)
                oStr = oStr + '\n' + typStr
                if CleanUpInput:
        ...
        return oStr
    #### DDL section
    def GenerateDDL(self, schemaName, tableName, CleanUpInput=False):
    ...
    def GenerateDDL_VarLine(self, schemaName = '', DumpLastComma=False):
        ...
        if AttributesToLower_Case:
            ...
            keyname = self.key
        numspaces = max(0, 70 -(len(self.key) + len(varChoiceString) + len(ddlvar_line)))
        if DumpLastComma:#gets rid of trailing commas
            varline_str =  '\t"%s" %s %s%s--%s' %(keyname, ddlvar_line, varChoiceString, ' '*numspaces, self.export())
        else:
            varline_str =  '\t"%s" %s,%s%s--%s' %(keyname, ddlvar_line, varChoiceString, ' '*numspaces, self.export())
        if ddlvar_line == 'NULL':
            return ''#--' + varline_str
        ...
        for varType in self.typeLst:
            ...
            if varType == 'dict':
                if self.key[-3:] == '_ts':
                    ...
                    oStr = self.DDL_TableName(schemaName, self.table_name)
                    oLst.append(oStr)
                    continue
            if varType == 'list':
    ...
    def GenerateDDL_CreateType(self, schemaName, table_name):
        print ('CreateType %s dict: [n=%i]' %(self.DDL_TableName(schemaName, table_name), len(self.d)))
        if self.Generate_DDL_TableTypeOrScalar() == 'type':
            Lst = ['CREATE TYPE %s AS RECORD (' %(self.DDL_TableName(schemaName, table_name))]
            ...
            if keyLst:
                ...
                for k in keyLst:
                    ...
                    ddl_varline = self.d[k].GenerateDDL_VarLine(schemaName, DumpLastComma=dumpLastComma)
                    if ddl_varline: 
            ...
            return Rstr
        #elif self.d:
        return '-- No Type Generation for Leaf Data Node: %s Types: %s' %(self, str(self.typeLst))
    def MakeSubmittableTable(self, inStr):
        #this is only relevant if the ddl processor doe not accept comments correctly. IT strips away the comments and makes a single line entry.
        inLst = inStr.split('\n')
        oLst = []
        for l in inLst:
            l = l.split('--')[0].strip()
    ...
    def GenerateDDL_CreateTable(self, schemaName, tableName):
        if self.Generate_DDL_TableTypeOrScalar() == 'type':
            ...
            for k in keyLst:
                if k == lastKey:
                    dumpLastComma = True
                v = self.d[k].GenerateDDL_VarLine(schemaName, DumpLastComma=dumpLastComma)
                if v:
                    Lst.append(v)
    ...
    def __str__(self):
        if self.histo:
            ...
            histoOutput = ''
        selfStr = "DataNode: %s : %s : n= %i%s  %s path:{%s}" %(self.key, self.typeLst, self.n, self.ListDictDesc, histoOutput, self.path)
        return selfStr
    def export(self):
        return str(self)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        rstr = '--verbose --sampler 1 --limit 1000 --schema_name schema'
        sys.argv = ['SchemaDiscovery.py'] + rstr.split(' ')
        #ifn = '../AirSage_etl/testdata/airsage.json'
        #ifn = '../AirSage_etl/airsage_test.json'
        #table_name = 'airsage2'
        ifn = '/Users/dominicsuciu/src/EclipseWorkspace/SCpyTools/src/twitter_etl/testdata/esnTwit_test.json'  ##testtw.json' #
        ifn = 'testdata/mini19238.json'
        table_name = 'test'
        
        sys.argv = sys.argv + ['--table_name', table_name, ifn]
        print('DebugRun: %s'%' '.join(sys.argv))
    ...
    numf = len(FileLst)
    n = 0;f = 0; n_f = 0
    RootNode = DataNode(root, table_name=table_name)
    for inpath in FileLst:
        ...
        for line in infp:
            ...
            l+=1
            if l%1000 ==0 : 
                print ('Files %s %i of %i Total Recs Processed %i ' %(ifn, f, numf, n))
                #print line
            #print "%imod%i %i"%(l, sampler, l%sampler)
            if l%sampler > 0: continue
            if n > MaxNumRecsToCount:break
            if line == '\lf': continue
            try:
                #print '>>%s<<'%line[-2:]
                line = line.replace(',  ]', ']')
                ...
                continue
            if maxrecsperfile and n_f > maxrecsperfile: break
            n+=1
    ...
    testOut =  FileLst_str + '\n' + testOut
    ddlStr = RootNode.GenerateDDL(schema_name, table_name, CleanUpInput=DDL_CleanUpInput)
    print(ddlStr)
